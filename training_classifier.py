...
from models.nets import GraphNN_KNN_v1, EdgeClassifier_v1
...
from utils.viz_utils import RunningAverageMeter, plot_aucs
import matplotlib.pyplot as plt
from tqdm import tqdm
import utils.classification_losses
from utils.sys_utils import get_freer_gpu
from roc_auc.roc_auc_fast import fast_auc
...
def main(
    ...
    key = "abUSnAytqEzSzLOxNLP1ohibs"
    experiment = Experiment(key, project_name=project_name, workspace=workspace) 
    device = torch.device(device)
    showers = torch.load(datafile)
   # showers_train, showers_test = train_test_split(showers, random_state=1337)
    showers_train, showers_test, showers_val = showers[:], showers[:], showers[:]
    ...
    val_loader = DataLoader(showers_val, batch_size=1, shuffle=True)
    ...
    for epoch in tqdm(range(epochs)):
        ...
        for shower in train_loader:
            ...
            print(edge_labels_predicted)
            #print(np.any(np.isnan(edge_labels_predicted)))
            #print(np.all(np.isfinite(edge_labels_predicted)))
        ...
        for shower in test_loader:
            ...
            roc_auc = fast_auc(y_true.astype(np.float64), y_pred.astype(np.float64))

# - fast_auc(y_true.astype(np.float64), -shower.edge_attr.view(-1).detach().cpu().numpy().astype(np.float64))

            plane_roc_auc.append(roc_auc)            
        ...
        #np.save(rocauc_file, np.array(plane_roc_auc))
        #experiment.log_asset('roc_auc_90_last.npz', np.array(plane_roc_auc))
        ...
        if loss_test.val < best_loss:
                print('saving')
               # best_roc_auc = roc_auc_test.val
                best_loss = loss_test.val
